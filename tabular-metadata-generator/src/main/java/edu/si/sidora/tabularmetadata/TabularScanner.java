...
/**
 * Scanning workflow. Handed a {@link CSVParser}, this class will scan through
 * it and supply the values of fields to "rows" of {@link Heuristic}s cloned
...
public class TabularScanner extends AbstractIterator<CSVRecord> {
	...
	public TabularScanner(final Iterator<CSVRecord> parser, final TypeDeterminingHeuristic<?> typeStrategy,
		...
		final int numColumns = internalScanner.hasNext() ? internalScanner.peek().size() : 0;

		log.debug("Found {} columns in our data.", numColumns);
		// create a "row" of type strategy instances of the same length as the
		// rows in our data
		...
		// create a "row" of range strategy instances of the same length as the
		// rows in our data
		...
		// create a "row" of enum strategy instances of the same length as the
		// rows in our data
	...
	protected CSVRecord computeNext() {
		if (internalScanner.hasNext()) {
			...
			for (final String lex : nextRecord) {
				...
				final boolean shouldContinue = rangeStrategiesShouldContinue || typeStrategiesShouldContinue
						|| enumStrategiesShouldContinue;
				if (!shouldContinue) {
	...
	/**
	 * Scan rows in our data up to a limit, exhausting values from the internal
	 * parser as we do. <br/>
	 * WARNING: Do not call this more than once on a {@link TabularScanner}. The
	 * internal parser of a scanner cannot be reset.
	 * 
	...
	/**
	 * Use this to recover and interrogate the type determining strategy
	 * instances used in scanning.
	 * 
	...
	/**
	 * Use this to recover and interrogate the range determining strategy
	 * instances used in scanning.
	 * 
	...
	/**
	 * Use this to recover and interrogate the enumerated-values determining
	 * strategy instances used in scanning.
	 * 
	 * @return the row of strategies used to determine the enumerated values
	 *         presented in fields
	 */
