...
public class TabularMetadataGeneratorTest {
	...
	private Predicate<Map<DataType, Set<String>>> matchesMockEnumeratedValues = new Predicate<Map<DataType, Set<String>>>() {
	...
	private Predicate<Map<DataType, Range<?>>> matchesTestRanges = new Predicate<Map<DataType, Range<?>>>() {
	...
	@Test
	public void testOperationWithOnlyHeaders() throws IOException {
		log.trace("Entering testOperationWithOnlyHeaders()...");
		when(mockHeaderHeuristic.results()).thenReturn(true);
		final URL mockURL = mockURL(testHeaders.getBytes());
		final TabularMetadataGenerator testParser = newGenerator(mockRangeStrategy, mockEnumStrategy, mockTypeStrategy);
		testParser.setHeaderStrategy(mockHeaderHeuristic);
		final TabularMetadata results = testParser.getMetadata(mockURL);
